...
class ManageBudgetedAmountInput(PlanIdInput):
    ...
    from_category_id: Optional[str] = Field(
        None, description="The ID of the category to move money from. Required for 'move' action."
    )
    to_category_id: Optional[str] = Field(
        None, description="The ID of the category to move money to, or the category to assign to."
    )
    ...
    def check_fields_for_action(cls, values):
        ...
        if action == "assign":
            ...
            if values.get("from_category_id"):
                raise ValueError(
                    "'from_category_id' should not be provided for the 'assign' action."
                )
        elif action == "move":
            if not values.get("from_category_id") or not values.get("to_category_id"):
                raise ValueError(
                    "'from_category_id' and 'to_category_id' are required for the 'move' action."
                )
...
class ManagePayeesInput(PlanIdInput):
    ...
    name: Optional[str] = Field(
        None, description="The new name for the payees. Required for 'rename' action."
    )
...
class ListAccountsInput(PlanIdInput):
    pass


class ListCategoriesInput(PlanIdInput):
    pass
...
class TransactionUpdate(BaseModel):
    ...
    approved: Optional[bool] = Field(
        None, description="Whether or not the transaction is approved."
    )
    flag_color: Optional[str] = Field(
        None,
        description="The flag color of the transaction.",
...
class BulkManageTransactionsInput(PlanIdInput):
    action: BulkManageTransactionsAction = Field(..., description="The action to perform.")
    create_transactions: Optional[List[NewTransactionModel]] = Field(
        None, description="A list of transactions to create. Required for 'create' action."
    )
    update_transactions: Optional[List[TransactionUpdate]] = Field(
        None, description="A list of transactions to update. Required for 'update' action."
    )
    delete_transaction_ids: Optional[List[str]] = Field(
        None, description="A list of transaction IDs to delete. Required for 'delete' action."
    )
...
class ListTransactionsInput(PlanIdInput):
    account_id: Optional[str] = Field(None, description="Filter to a single account.")
    category_id: Optional[str] = Field(
        None, description="Filter to a single category (e.g. all 'Groceries' spending)."
    )
    payee_id: Optional[str] = Field(
        None, description="Filter to a single payee (e.g. all spending at 'Amazon')."
    )
    month: Optional[str] = Field(
        None, description="Filter to a single month (YYYY-MM-DD, first of the month)."
    )
    since_date: Optional[str] = Field(
        None,
        description="Only return transactions on or after this date (YYYY-MM-DD). Ignored when 'month' is set.",
    )
    ...
    def check_exclusive_fields(cls, values):
        filters = [k for k in ("account_id", "category_id", "payee_id", "month") if values.get(k)]
        if len(filters) > 1:
            raise ValueError(
                'Provide at most one of "account_id", "category_id", "payee_id", or "month".'
            )
...
class ManageScheduledTransactionInput(PlanIdInput):
    action: ManageScheduledTransactionAction = Field(..., description="The action to perform.")
    transaction_id: Optional[str] = Field(
        None, description="The ID of the scheduled transaction to update or delete."
    )
    transaction_data: Optional[ScheduledTransaction] = Field(
        None, description="The data for the scheduled transaction to create or update."
    )
    ...
    def check_fields_for_action(cls, values):
        ...
        if action == "create":
            ...
            if not transaction_id or not transaction_data:
                raise ValueError(
                    "'transaction_id' and 'transaction_data' are required for the 'update' action."
                )
        elif action == "delete":
            ...
            if transaction_data:
                raise ValueError(
                    "'transaction_data' should not be provided for the 'delete' action."
                )
...
class LookupEntityByIdInput(PlanIdInput):
...
class GetMonthInfoInput(PlanIdInput):
    month: Optional[str] = Field(
        None,
...
class LookupPayeeLocationsInput(PlanIdInput):
    location_id: Optional[str] = Field(
        None, description="The ID of a specific payee location to retrieve."
    )
    payee_id: Optional[str] = Field(None, description="The ID of a payee to list locations for.")
